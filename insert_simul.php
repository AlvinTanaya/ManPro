...
// Check if form is submitted and $_POST is set
    // Retrieve the selected RawDataName from the form
    $rawData = $rawDataName;
    echo '<br>' . $rawData . '<br>';

    $sql2 = "SELECT rawDataName FROM rawdata WHERE rawdata.id = '" . $rawData . "'";
    ...
    if ($result) {
        ...
        $simulationName = $simulname;
        echo $simulationName;
    } else {
        echo "Error in SQL query: " . mysqli_error($conn);
    }

    $rawData_baru = json_decode($data['rawDataName'], true);
?>
